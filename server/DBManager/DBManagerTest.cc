...
DEFINE_string(test_path, "/root/Kingslanding/OnlineWhiteBoard/server/DBManager/", "the path of the test sql file");
...
protected:
  ...
  virtual void SetUp() {
    ...
    sprintf (sql, "load data local infile  '%sMeeting.txt' into table Meeting", FLAGS_test_path.c_str());
    if (mysql_query(conn_, sql)) {
     LOG(ERROR) << mysql_error(conn_);
    }
    sprintf (sql, "load data local infile  '%sMeetingUser.txt' into table MeetingUser", FLAGS_test_path.c_str());
    if (mysql_query(conn_, sql)) {
     LOG(ERROR) << mysql_error(conn_);
    }
    sprintf (sql, "load data local infile  '%sMeetingDocument.txt' into table MeetingDocument", FLAGS_test_path.c_str());
    if (mysql_query(conn_, sql)) {
     LOG(ERROR) << mysql_error(conn_);
    }
  }
  virtual void TearDown() {
    char sql[500];
    strcpy(sql, "delete from Meeting");
    if (mysql_query(conn_, sql)) {
     LOG(ERROR) << mysql_error(conn_);
    } 
    strcpy(sql, "delete from MeetingUser");
    if (mysql_query(conn_, sql)) {
     LOG(ERROR) << mysql_error(conn_);
    }
    strcpy(sql, "delete from MeetingDocument");
    if (mysql_query(conn_, sql)) {
     LOG(ERROR) << mysql_error(conn_);
    }
...
TEST_F(DBManagerTest, SetMeetingID) {
  ...
  char sql[500];
  char result[50];
  sprintf (sql, "select MeetingID  from Meeting where id = %d", id);
  if (mysql_query(conn_, sql)) {
     LOG(ERROR) << mysql_error(conn_);
   } else {
...
TEST_F(DBManagerTest, SetMeetingState) {
  EXPECT_TRUE(db_instance->SetMeetingState("072B030BA126B2F4B2374F342BE9ED44", 1));
  char sql[500];
  int result = 0;
  sprintf (sql, "select state  from Meeting where id = 60");
  if (mysql_query(conn_, sql)) {
     LOG(ERROR) << mysql_error(conn_);
   } else {
     MYSQL_RES *res_ = mysql_store_result(conn_);
...
TEST_F(DBManagerTest, AddMeetingPort) {
  EXPECT_TRUE(db_instance->AddMeetingPort("072B030BA126B2F4B2374F342BE9ED44", 1));
  char sql[500];
  int result = 0;
  sprintf (sql, "select Port  from Meeting where id = 60");
  if (mysql_query(conn_, sql)) {
     LOG(ERROR) << mysql_error(conn_);
   } else {
     MYSQL_RES *res_ = mysql_store_result(conn_);
...
TEST_F(DBManagerTest, GetMeetingPort) {
  EXPECT_EQ(8080, db_instance->GetMeetingPort("7F39F8317FBDB1988EF4C628EBA02591"));
}

TEST_F(DBManagerTest, SetDataRef) {
  EXPECT_TRUE(db_instance->SetDataRef("072B030BA126B2F4B2374F342BE9ED44", 1));
  char sql[500];
  int result = 0;
  sprintf (sql, "select DataRef  from Meeting where id = 60");
  if (mysql_query(conn_, sql)) {
     LOG(ERROR) << mysql_error(conn_);
   } else {
     MYSQL_RES *res_ = mysql_store_result(conn_);
...
TEST_F(DBManagerTest, GetDataRef) {
  EXPECT_EQ(1234567, db_instance->GetDataRef("7F39F8317FBDB1988EF4C628EBA02591"));
}

TEST_F(DBManagerTest, AddDocuemnt) {
  EXPECT_TRUE(db_instance->AddDocument("072B030BA126B2F4B2374F342BE9ED44", "abcd"));
  char sql[500];
  char result[100];
  sprintf (sql, "select path  from MeetingDocument where DocumentID > 12 \
...
TEST_F(DBManagerTest, GetDocument) {
  DocumentInfo res = db_instance->GetDocument("14BFA6BB14875E45BBA028A21ED38046", 5);
  EXPECT_EQ(0, strcmp(res.path, "567890"));
  EXPECT_EQ(5, res.serial_number);
}

TEST_F(DBManagerTest, GetCurrentDocument) {
  DocumentInfo res = db_instance->GetCurrentDocument("14BFA6BB14875E45BBA028A21ED38046");
...
TEST_F(DBManagerTest, GetHistoryDocuments) {
  int size;
  DocumentInfo *info = db_instance->GetHistoryDocuments("A3F390D88E4C41F2747BFA2F1B5F87DB", size);
  EXPECT_EQ(6, size);
...
TEST_F(DBManagerTest, AddMeetingUser) {
  EXPECT_EQ(1, db_instance->AddMeetingUser("072B030BA126B2F4B2374F342BE9ED44", "abcd", 3));
  char sql[500];
  int result = 0;
  sprintf (sql, "select state  from Meeting  where id = 60");
  if (mysql_query(conn_, sql)) {
  ...
  EXPECT_EQ(1, result);
  EXPECT_EQ(3, db_instance->AddMeetingUser("44F683A84163B3523AFE57C2E008BC8C", "abcd", 3));
  EXPECT_EQ(2, db_instance->AddMeetingUser("123456", "abcd", 3));
}

TEST_F(DBManagerTest, GetUserState) {
  EXPECT_EQ(2, db_instance->GetUserState("FC490CA45C00B1249BBE3554A4FDF6FB", "cd2" ));
}

TEST_F(DBManagerTest, UpdateUserState) {
  EXPECT_EQ(1, db_instance->UpdateUserState("FC490CA45C00B1249BBE3554A4FDF6FB", "cd3", 1));
  char sql[500];
  int result = 0;
  sprintf (sql, "select State  from MeetingUser  where id = 16");
  if (mysql_query(conn_, sql)) {
     LOG(ERROR) << mysql_error(conn_);
   } else {
     MYSQL_RES *res_ = mysql_store_result(conn_);
  ...
  EXPECT_EQ(1, result);
  EXPECT_EQ(1, db_instance->UpdateUserState("FC490CA45C00B1249BBE3554A4FDF6FB", "cd2", 3));
    sprintf (sql, "select State  from MeetingUser  where id = 15");
  if (mysql_query(conn_, sql)) {
     LOG(ERROR) << mysql_error(conn_);
   } else {
     MYSQL_RES *res_ = mysql_store_result(conn_);
  ...
  EXPECT_EQ(3, result);
  EXPECT_EQ(2, db_instance->UpdateUserState("FC490CA45C00B1249BBE3554A4FDF6FB", "cd1", 3));
    sprintf (sql, "select State  from MeetingUser  where id = 14");
  if (mysql_query(conn_, sql)) {
     LOG(ERROR) << mysql_error(conn_);
   } else {
     MYSQL_RES *res_ = mysql_store_result(conn_);
...
TEST_F(DBManagerTest, GetUserList) {
  int size;
  UserInfo *info = db_instance->GetUserList("FC490CA45C00B1249BBE3554A4FDF6FB", size);
  EXPECT_EQ(3, size);
...
TEST_F(DBManagerTest, UpdateUserTime) {
  ...
  EXPECT_TRUE( db_instance->UpdateUserTime("FC490CA45C00B1249BBE3554A4FDF6FB", "cd1"));
  char sql[500];
  long result = 0;
  sprintf (sql, "select HBTime  from MeetingUser  where id = 14");
  if (mysql_query(conn_, sql)) {
     LOG(ERROR) << mysql_error(conn_);
   } else {
     MYSQL_RES *res_ = mysql_store_result(conn_);
...
TEST_F(DBManagerTest, DeleteDeadUser) {
  ...
  char sql[500];
  sprintf (sql, "select *  from MeetingUser  where HBTime < 1000000");
  if (mysql_query(conn_, sql)) {
     LOG(ERROR) << mysql_error(conn_);
   } else {
     MYSQL_RES *res_ = mysql_store_result(conn_);
...
TEST_F(DBManagerTest, GetDeadMeeting) {
  ...
  char sql[500];
  char result[10][50];
  sprintf (sql, "select MeetingID  from Meeting  where state = 2 order by id");
  if (mysql_query(conn_, sql)) {
     LOG(ERROR) << mysql_error(conn_);
   } else {
     MYSQL_RES *res_ = mysql_store_result(conn_);
...
TEST_F(DBManagerTest, TransferAuth) {
  EXPECT_TRUE(db_instance->TransferAuth("7F39F8317FBDB1988EF4C628EBA02591"));
  char result[100];
  char sql[500];
  sprintf (sql, "select UserID  from MeetingUser  where State=3 and MeetingID \
  ...
  EXPECT_TRUE(db_instance->TransferAuth("44F683A84163B3523AFE57C2E008BC8C"));
  sprintf (sql, "select UserID  from MeetingUser  where State=3 and MeetingID \
  ...
  EXPECT_TRUE(db_instance->TransferAuth("03AFDBD66E7929B125F8597834FA83A4"));
  sprintf (sql, "select UserID  from MeetingUser  where State=3 and MeetingID \
  ...
  EXPECT_TRUE(db_instance->TransferAuth("EA5D2F1C4608232E07D3AA3D998E5135"));
  sprintf (sql, "select UserID  from MeetingUser  where State=3 and MeetingID \
  ...
  EXPECT_TRUE(db_instance->TransferAuth("FC490CA45C00B1249BBE3554A4FDF6FB"));
  sprintf (sql, "select UserID  from MeetingUser  where State=3 and MeetingID \
