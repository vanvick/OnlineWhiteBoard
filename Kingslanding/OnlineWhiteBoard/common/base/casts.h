...
#ifndef COMMON_BASE_CASTS_H_
#define COMMON_BASE_CASTS_H_
...
#include "common/base/template_util.h"
...
inline To implicit_cast(typename common::base::identity_<To>::type to) {
  return to;
}
...
inline To implicit_cast(
        typename common::base::identity_<From>::type const &f) {
  return f;
}
...
inline To down_cast(From& f) {
  COMPILE_ASSERT(common::base::is_reference<To>::value,
          target_type_not_a_reference);
  typedef typename common::base::remove_reference<To>::type* ToAsPointer;
  if (false) {
...
inline Enum loose_enum_cast(int e_val) {
  if (!loose_enum_test<Enum>(e_val)) {
      common::base::internal::WarnEnumCastError(e_val);
  }
