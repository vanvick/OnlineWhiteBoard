...
#include "common/base/strings/split.h"
...
#include "common/base/strtoint.h"
#include "common/base/strings/ascii_ctype.h"
#include "common/base/strings/util.h"
#include "common/base/hash/hash.h"
...
StringPiece GenericFind(
  ...
  if (found_pos != static_cast<int>(StringPiece::npos)) {
...
using ::common::strings::SkipEmpty;
using ::common::strings::delimiter::AnyOf;
using ::common::strings::delimiter::Limit;
...
static int ClipStringHelper(const char* str, int max_len, bool use_ellipsis) {
  if (static_cast<int>(strlen(str)) <= max_len)
    return -1;
...
void SplitStringIntoNPiecesAllowEmpty(const string& full,
  ...
  if (pieces == 0) {
    ...
    AppendTo(result, common::strings::Split(full, AnyOf(delim)));
  } else {
    ...
    AppendTo(result, common::strings::Split(full, Limit(AnyOf(delim), limit)));
  }
}
...
void SplitStringPieceToVector(const StringPiece& full,
  ...
  if (omit_empty_strings) {
    AppendTo(vec, common::strings::Split(full, AnyOf(delim), SkipEmpty()));
  } else {
    AppendTo(vec, common::strings::Split(full, AnyOf(delim)));
  }
}
...
void SplitStringWithEscapingToIterator(const string& src,
                                       const common::strings::CharSet& delimiters,
...
bool SplitStringIntoKeyValues(const string& line,
  ...
  if (end_key_pos == static_cast<int>(string::npos)) {
  ...
  if (begin_values_pos == static_cast<int>(string::npos)) {
