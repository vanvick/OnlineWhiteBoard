...
#include "common/base/strings/stringprintf.h"
...
char* DoubleToBuffer(double value, char* buffer) {
  ...
  double str_value = strtod(buffer, NULL);
  // comparing flating point with == or != is unsafe, so we using < or >
  // replace !=
  if ( str_value > value || str_value < value) {
    snprintf_result =
...
char* FloatToBuffer(float value, char* buffer) {
  ...
  float parsed_value;
  if (!safe_strtof(buffer, &parsed_value) ||
